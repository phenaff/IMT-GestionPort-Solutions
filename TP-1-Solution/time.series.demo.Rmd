...
On commence par obtenir une liste des tickers du CAC40, puis on lit une série.
```{r, echo=TRUE}
tickers <- get.tickers(folder = "CAC40")
knitr::kable(head(tickers), row.names=FALSE, booktabs=TRUE, col.names="Ticker", caption="Echantillon de tickers du CAC40")
```
...
```{r, fig-11, echo=FALSE, fig.width=10, fig.cap="Airbus"}
plot(ts.air, main='Airbus')
```
...
```{r, cor-0, echo=TRUE, cache=FALSE, warning=FALSE, message=FALSE, cache=TRUE} 
ts.all <- get.all.ts(folder="EuroStoxx50", returns=TRUE, combine=TRUE)
```

On calcule la matrice de corrélation sur plusieurs intervalles de 1 an:

```{r, cor-1, echo=TRUE}
nb.obs <- 252
dt.start <- dmy("01Jan2009")
idx.start <- closest.index(ts.all, dt.start)
idx <- seq(idx.start, length.out=nb.obs)
cor.EX50 <- cor(ts.all[idx,])
```

```{r, fig.cap="Corrélation de l'EuroStoxx50, 2009"}
corrplot(cor.EX50, type="upper", order="hclust", tl.pos='n')
```

```{r, cor-11, echo=TRUE}
dt.start <- dmy("01Jan2012")
idx.start <- closest.index(ts.all, dt.start)
idx <- seq(idx.start, length.out=nb.obs)
cor.EX50 <- cor(ts.all[idx,])
```

```{r, fig.cap="Corrélation de l'EuroStoxx50, 2012"}
corrplot(cor.EX50, type="upper", order="hclust", tl.pos='n')
```
